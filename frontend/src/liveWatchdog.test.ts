...
describe('watchdogAfterPoll', () => {
  it('resets the clock when a working span begins', () => {
    ...
    const idle = watchdogAfterPoll(start, { workingSince: null, turns: [], now: 5000 })
  ...
  it('resets the clock when a new mesa turn arrives, never on a user turn', () => {
    const start = initialWatchdog(1, 1000)
    const one = watchdogAfterPoll(start, { workingSince: null, turns: [turn(3)], now: 2000 })
    expect(one.lastActivityAt).toBe(2000)
    expect(one.lastTurnId).toBe(3)
    // Seeing the same turn again is not activity.
    const same = watchdogAfterPoll(one, { workingSince: null, turns: [turn(3)], now: 3000 })
  ...
  it('a span ending is remembered but is not activity', () => {
    ...
    const ended = watchdogAfterPoll(working, { workingSince: null, turns: [], now: 3000 })
...
describe('shouldNoticePermission', () => {
  it('fires on a blocked job that has not been reported this span', () => {
    expect(shouldNoticePermission({ blocked: 'permission prompt', alreadyNoticed: false })).toBe(true)
    expect(shouldNoticePermission({ blocked: 'permission prompt', alreadyNoticed: true })).toBe(false)
    expect(shouldNoticePermission({ blocked: null, alreadyNoticed: false })).toBe(false)
  })
})
