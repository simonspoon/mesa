...
[ "$(jq -r 'map(.action) | join(",")' <<<"$STDOUT")" = "todo-watcher,inbox-watcher,agent-spawn,live-agent,live-summary,live-dream,retro" ] ||
  fail "GET /api/config must list all seven actions in order: $STDOUT"
