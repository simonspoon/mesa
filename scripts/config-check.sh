...
[ "$(jq -r 'map(.action) | join(",")' <<<"$STDOUT")" = "todo-watcher,inbox-watcher,agent-spawn,live-agent,live-summary,live-dream" ] ||
  fail "GET /api/config must list all six actions in order: $STDOUT"
