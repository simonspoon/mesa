...
The two flags are **independent** — neither implies the other, and each drives
its own interval loop with its own tick constant. `--watch-inbox` alone never
claims a task or dispatches `/execute-mesa-task`.
