...
[ "$(jq -r 'map(.action) | join(",")' <<<"$STDOUT")" = "todo-watcher,inbox-watcher,agent-spawn,live-agent,live-summary" ] ||
  fail "GET /api/config must list all five actions in order: $STDOUT"
